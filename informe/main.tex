...
{\scriptsize
\begin{verbatim}
# Modelo de costos 4.2 - precios de las Tablas 3.2a y 3.2b (us-east-1, USD, 20-09-2026)
import math

H = 730; SEC = H * 3600                          # horas y segundos por mes
C_REQ, C_GBS = 0.20 / 1e6, 0.0000166667          # Lambda x86: USD/solicitud y USD/GB-s
...
P_FARGATE = 0.5 * 0.04048 + 1.0 * 0.004445       # tarea Fargate 0,5 vCPU y 1 GB, USD/h

def lambda_cost(N, m=0.5, d=0.2):
    """Costo mensual de Lambda: N solicitudes, m GB, d segundos."""
    return (max(0, N - FREE_REQ) * C_REQ
            + max(0, N * m * d - FREE_GBS) * C_GBS)

def fleet_cost(N, p=P_H["1a"], cap=20, f=4, kmin=2, extra=0.0):
    """Costo mensual de la flota fija: k unidades a p USD/h, cap req/s c/u."""
    k = max(kmin, math.ceil(f * (N / SEC) / cap - 1e-12))
    return k * p * H + extra

def first_cross(fleet, m=0.5, d=0.2, hi=400e6, step=1e4):
    """Menor N con costo Lambda >= costo flota; None si nunca ocurre."""
    N = step
    while N <= hi:
        if lambda_cost(N, m, d) >= fleet(N):
            return N
        N += step
    return None
...
if __name__ == "__main__":
    ...
    print("Equilibrio base (M/mes):", first_cross(base) / 1e6)
    for m in (0.25, 0.5, 1.0):                   # sensibilidad memoria x duracion
        print(m, [first_cross(base, m, d) for d in (0.1, 0.2, 0.5)])
    print("Reserva 3 anos:", first_cross(lambda N: fleet_cost(N, p=P_H["3a"])))
    print("Bajo demanda:", first_cross(lambda N: fleet_cost(N, p=P_H["od"])))
    print("Con NAT:", first_cross(lambda N: fleet_cost(N, extra=0.045 * H)))
    print("Fargate:", first_cross(lambda N: fleet_cost(N, p=P_FARGATE, cap=25)))
\end{verbatim}
}
